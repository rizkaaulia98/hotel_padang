...
function basemap(){   // GOOGLE MAP
  map = new google.maps.Map(document.getElementById('map'),
      {
        zoom: 13,
        center: new google.maps.LatLng(-0.3027177, 100.3544704),
...
content.push('<p><div class="color a"></div>Mandiangin Koto Selayan District</p>');
content.push('<p><div class="color b"></div>Guguak Panjang District</p>');
content.push('<p><div class="color c"></div>Aur Birugo Tigo Baleh District</p>');
...
function listHotel(){ // Menu Hotel
  ...
  $.ajax({
  url: server+'/_data_hotel1.php', data: "", dataType: 'json', success: function(rows)
  {
      for (var i in rows){
        var row = rows[i];
        var id = row.id;
...
function listgeom(ids){
        hapusInfo();
        $.ajax({
            url: server+'tampilkanrute.php?id='+ids, data: "", dataType: 'json', success: function(rows)
            {
