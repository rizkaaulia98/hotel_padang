...
//    for ($a=0; $a < count($o_hotel) ; $a++) {
    for ($a=0; $a < 5 ; $a++) {
        ...
        $sql_hotel = mysqli_query($conn, "select hotel.id, hotel.name, hotel.address, hotel.cp, hotel.ktp, hotel.marriage_book, hotel.mushalla, hotel_type.name as type_hotel, ST_X(ST_Centroid(hotel.geom)) AS lng, ST_Y(ST_CENTROID(hotel.geom)) As lat from hotel left join hotel_type on hotel_type.id=hotel.id WHERE hotel.id = '$id_hotel'");
        while($baris = mysqli_fetch_array($sql_hotel))
            {
