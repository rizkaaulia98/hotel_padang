...
include ('../../../connect.php');
	...
	if(($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/gif"  || $jenis_gambar=="image/png") && ($_FILES["image"]["size"] <= 90000000)){
		...
		$sql = mysqli_query($conn, "insert into hotel_gallery(serial_number, id, gallery_hotel) values('$serial_number','$id','$name')");
			if($sql){
				header("location:../indexu.php?page=hotel_owner&id=$id");
					...
					echo "<script language='javascript'>alert('The Picture Isn't Falid');</script>";
			}
	}
